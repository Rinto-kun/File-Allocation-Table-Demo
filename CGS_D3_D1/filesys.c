...
diskblock_t* findEmptySpace(){
    for(int i=3; i<MAXBLOCKS;i++){
        if(FAT[i]==UNUSED) {
            FAT[i] = ENDOFCHAIN;
            copyFAT();
            return &virtualDisk[i];
        }
    }
    // Somehow the whole FAT is full.
}
    ...
    for (char *rest = NULL, *token = strtok_r(str, "/", &rest); token != NULL; token = strtok_r(NULL, "/", &rest)) {
        ...
        for(int i=2; i<DIRENTRYCOUNT;i++){
            if(curBlock.dir.entrylist[i].used==FALSE){
                curBlock.dir.entrylist[i].used = TRUE;
                curBlock.dir.entrylist[i].isdir= TRUE;
                strcpy(curBlock.dir.entrylist[i].name,token);

                immediateDirIndex = dirMaker(token);   
                curBlock.dir.entrylist[i].firstblock = immediateDirIndex;
...
void myrmdir(char* path){
    ...
    for(int i=2; i<DIRENTRYCOUNT;i++){
        if(strcmp(block.dir.entrylist[i].name,"\0")!=0)
            printf("The directory is not empty. Please, empty it first before removing.\n");
            return;
    }
